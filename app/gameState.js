...
export function resetGame(state, mode = "pvp", difficulty = null, timed = false) {
  ...
  // Timer sempre azzerato a 5 minuti; il timestamp di riferimento
  // serve solo in modalità a tempo (usato da startTimer/switchTimer)
  state.timerX = 300;
  state.timerO = 300;
  state.lastMoveTime = timed ? Date.now() : null;
}
